...
class Generator:
    ...
    def generate(self, prompt: str) -> str:
        ...
        # FIX: Truncate nếu vượt giới hạn + log cảnh báo để dễ debug
        if inputs["input_ids"].shape[1] > max_input:
                        logger.warning(
                                            f"Prompt quá dài ({inputs['input_ids'].shape[1]} tokens > {max_input}), "
                                            "cắt bớt phần đầu context để giữ câu hỏi."
                        )
        ...
        # FIX: Thêm top_p để sampling ổn định hơn; đảm bảo do_sample nhất quán
    ...
    def stream(self, prompt: str):
        ...
        if inputs["input_ids"].shape[1] > max_input:
                        logger.warning(
                                            f"[stream] Prompt quá dài ({inputs['input_ids'].shape[1]} tokens), cắt bớt."
                        )
                        inputs = {k: v[:, -max_input:] for k, v in inputs.items()}
